...
def size_if_newer(source, target):
    ...
    print("From size newer file src stat is",src_stat)
    try:
        target_ts = os.stat(target).st_mtime
        print("From target os", src_stat)
...
def sync_file(source, target, compress):
    print("From sync file source is",source)
    size = size_if_newer(source, target)
    print("From sync file size is",size)
    if size:
...
if __name__ == '__main__':
    # arg = parse_input()
    arg = sync_file("C:/Users/Kailikia/Documents/OpenCart","C:/Users/Kailikia/Desktop/Icons",True)
    print("arg",arg)
    # print('### Start copy ####')
